...
## Running it yourself and viewing it from your phone (beginner guide)
...
4. **Point the terminal at that folder.** Type `cd ` (with a space after it), then **drag the `MTB-openings` folder onto the terminal window** — it pastes the location for you. Press Enter. Example of what it looks like:
   ```bash
...
### Part 2 — Install Tailscale (the magic that lets your phone connect)
...
6. **On the host computer:** go to [tailscale.com/download](https://tailscale.com/download), install it, and sign in (Google or any account works — the free plan is plenty). After signing in, it gives this computer a name (something like `mtb-host`). Write that name down.
...
(On a Raspberry Pi or other Linux server, a systemd service does the same job — see "Deploying to a server" above.)
